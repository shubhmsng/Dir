...
def on_find():
    ...
    e.place(x = 80, y = 25)
    e.focus_set()
    Button(t2, text="Find All", relief='groove', underline=0, command=lambda:search_for(v.get(), textPad, t2, e) ).place(x = 120, y = 60)
...
def save_as():
    ...
    try:
        f = tkinter.filedialog.asksaveasfilename(initialfile ='Untitled.txt', defaultextension=".txt",filetypes=[("AllFiles","*.*"),("Text Documents","*.txt")])
        fh = open(f, 'w')
...
def update_line():
    ...
    if showlen.get() :
        ...
        infobar.config(text= 'Line: %s | Column: %s' %(currline,curcolumn), bg='#e2ffff')
    else:
        infobar.config(text='', bg='#e2ffff')
def update_ln(event):
    update_line()
...
def on_replace(event=NONE):
    ...
    e2.focus_set()
    Button(t3, text="Replace All", underline=0,relief='groove', command=lambda:replace(v1.get(),v2.get(),textPad, t3, e1,e2) ).place(x=120, y = 60)
    def close_search():
        textPad.tag_remove('match', '1.0', END)
        t3.destroy()
...
frame1 = Frame(root,  bg='#e2ffff', height=15)
...
textPad = Text(root,undo=True,bg="light yellow", font=helv, padx=10,pady=10)
